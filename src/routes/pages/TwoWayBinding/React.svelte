<script>
    import CodeHighlight from "$lib/CodeHighlight.svelte";
</script>

<h3>Two-Way Binding - React Way</h3>
<CodeHighlight code={"// Parent.jsx\n//...\nconst Whatever = () => {\n  const [checkboxState, setCheckboxState] = useState(true);\n\n  return <Checkbox value={checkboxState} handleChange={(value) => setCheckboxState(value)} />\n}"} />
<CodeHighlight code={"// Checkbox.jsx\nexport const Checkbox = ({value, handleChange}) =>\n  <input type='checkbox' value={value} onChange={(event) => handleChange(event.target.value)} />"} />
